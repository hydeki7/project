...
class Jenis_perizinan extends AUTH_Controller {
	...
	public function index() {
		...
		$data['dataJenisPerizinan'] = $this->M_jenis_perizinan->select_all_jenis_perizinan();

		$data['page'] = "jenis_perizinan";
		$data['judul'] = "Data Jenis Perizinan";
		$data['deskripsi'] = "Manage Data Jenis Perizinan";

		$data['modal_tambah_jenis_perizinan'] = show_my_modal('modals/modal_tambah_jenis_perizinan', 'tambah-jenis-perizinan', $data);

		$this->template->views('jenis_perizinan/home', $data);
	}

	public function tampil() {
		$data['dataJenisPerizinan'] = $this->M_jenis_perizinan->select_all_jenis_perizinan();
		$this->load->view('jenis_perizinan/list_data', $data);
	}

	public function prosesTambah() {
		$this->form_validation->set_rules('nama_perizinan', 'Nama', 'trim|required');
		...
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_jenis_perizinan->insert($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Jenis Perizin "'.$data['nama_perizinan'] .'" Berhasil ditambahkan', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_err_msg('Data Jenis Perizin Gagal ditambahkan', '20px');
			}
		} else {
			$out['status'] = 'form';
	...
	public function update() {
		$data['userdata'] 	= $this->userdata;
		$id 				= trim($_POST['id']);
		$data['dataJenisPerizinan'] 	= $this->M_jenis_perizinan->select_by_id($id);

		echo show_my_modal('modals/modal_update_jenis_perizinan', 'update-jenis-perizinan', $data);
	}

	public function prosesUpdate() {
	$this->form_validation->set_rules('nama_jenis_perizinan', 'Nama_Jenis_Perizinan', 'trim|required');
		// $this->form_validation->set_rules('kota', 'Kota', 'trim|required');
		// $this->form_validation->set_rules('jk', 'Jenis Kelamin', 'trim|required');
		// $this->form_validation->set_rules('posisi', 'Posisi', 'trim|required');
		...
		$data = $this->input->post();
		if ($this->form_validation->run() == TRUE) {
			$result = $this->M_jenis_perizinan->update($data);

			if ($result > 0) {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Jenis Perizinan Berhasil diupdate', '20px');
			} else {
				$out['status'] = '';
				$out['msg'] = show_succ_msg('Data Jenis Perizinan Gagal diupdate', '20px');
			}
		} else {
			$out['status'] = 'form';
			$out['msg'] = show_err_msg(validation_errors());
